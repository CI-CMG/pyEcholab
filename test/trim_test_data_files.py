...
# This script will read one or more files and then write a small subsets of
# the data to a new set of data files.

# specify the start ping
start_ping = 0

# specify the end ping
end_ping = 49

# set the ping stride
ping_stride = 1

# specify the input file directory. All raw files in this directory will be
# processed.
in_path = './data/source/'

# specify the output file directory. The
out_path = './data/'
...
for in_file in in_files:
    ...
    for data_obj in raw_data_ojbs:
        ...
        outfile_name = os.path.normpath(out_path + os.path.sep + infile_root + '_s' + infile_ext)
        ...
        # You can pass either a dict, numpy array containing indices to write (aka index array),
        # or a boolean numpy array where the pings that map to the True elements will be written.
        # If you provide an index or boolean array, it is assumed that those values map to all
        # channels in the EKx0 object. You can pass a dict keyed by channel ID, that maps an
        # index or boolean array to that specific channel. The latter allows you control over
        # what pings are written to the individual channels.
