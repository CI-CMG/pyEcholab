...
one_file = ['C:/EK80 Test Data/EK80/CW/reduced/DY2000_EK80_Cal-D20200126-T062905.raw']
...
rewrite_chan_data = ek80rewrite.get_channel_data()
...
for channel_id in rewrite_chan_data:
    ...
    eg.axes.set_title("EK80 Original Sv " + str(orig_freq) + " kHz")

    fig = plt.figure()
    eg = echogram.Echogram(fig, rewrite_Sv, threshold=[-70,-34])
    eg.axes.set_title("EK80 Rewrite Sv " + str(rewrite_freq) + " kHz")
    ...
    fig2.suptitle("Ping " + str(rewrite_Sv.n_pings-1) + " EK80 comparison original vs pyEcholab2 re-write")
    plt.xlabel("Sv (dB)")
    plt.ylabel("Range (m)")
    plt.legend()
