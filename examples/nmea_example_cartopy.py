# -*- coding: utf-8 -*-
"""This example demonstrates plotting tracklines

REQUIRES THE CARTOPY PACKAGE

Installation instructions are available here:

https://scitools.org.uk/cartopy/docs/v0.15/installing.html#installing
...
def plot_trackline(lat, lon, raw_positions=None):
    ...
    plt.plot(lon, lat, color='r', marker='d', markerfacecolor='None',
             markeredgecolor='r')

    # If we've been given the raw positions, plot those as marks only.
    if raw_positions is not None:
        plt.plot(raw_positions[1], raw_positions[0], linestyle='None',
                 color='b', marker='x')
    plt.show()
    ...
    # Plot lon, lat values on the map
    plt.plot(lon, lat, color='r', marker='d', markerfacecolor='None',
            markeredgecolor='r', transform=ccrs.PlateCarree())
    
    # If we've been given the raw positions, plot those as marks only.
    if raw_positions is not None:
        plt.plot(raw_positions[1], raw_positions[0], linestyle='None',
                 color='b', marker='x',transform=ccrs.PlateCarree())
    plt.show()
