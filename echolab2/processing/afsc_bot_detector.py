...
class afsc_bot_detector(ping_data):
    ...
    Attributes:
        ...
        ratio_threshold - the maximum percent change of depth for a ping
        running_ave_size - number of pings to compute change in depth over for adjusting backstep
        change_threshold - the minimum change in depth over the running_ave_size number of pings
                          to apply an additional backstep
        adj_backstep_scaler - a multiplier to the backstep value, if depth changes are above
                          the change_threshold.  Default is 1, means no change in backstep.

    """

    def __init__(self, search_min=10, window_len=11, backstep=35, ratio_threshold=5,  
                                running_ave_size=5,  change_threshold=0.5,  adj_backstep_scaler=1):
        ...
        # specify the number of pings to compute the change in depth over for use in applying an 
        # additional backstep.  In steep conditions, an extra backstep is used.
        self.running_ave_size = running_ave_size
        
        # specify the threshold of change in depth over the running average, 
        # over which an addtional backstep is applied
        self.change_threshold = change_threshold
        
        # specify the adjustment factor applied to the backstep value, if the change is depth is great, 
        # A value 1 means no additional backstep is applied, which is the default
        self.adj_backstep_scaler = adj_backstep_scaler
    ...
    def detect(self, p_data, search_max=9999):
        ...
        for ping in p_data:
            ...
            if (not np.all(np.isnan(ping))):
                ...
                ave_data=[]
                if this_ping>self.running_ave_size-1:
                    ave_data=np.append(bot_line.data[this_ping-self.running_ave_size:this_ping], temp_data)
                    ...
                    if abs(change)>self.change_threshold:
                        threshold = max_Sv - self.backstep*self.adj_backstep_scaler
