...
def nt_to_unix(nt_timestamp_tuple, return_datetime=True):
    ...
    lowDateTime, highDateTime = nt_timestamp_tuple
    #sec past nt epoch = interval_count * 100ns/interval * 1e-9s/ns
    ...
    if return_datetime:
#        return datetime.datetime.fromtimestamp(sec_past_unix_epoch, tz=pytz_utc)
...
def datetime_to_unix(datetime_obj):
    ...
    #Convert to desired destination timezone if needed
    # if tz is not None:
    #     datetime_obj = datetime_obj.astimezone(tz)
    
    #
    #utc_tuple = datetime_obj.utctimetuple()
    
    #our UTC offset.  We do not need to take DST into account, utctimetuple()
    #returns a tuple in the UTC timezone, and UTC never has DST
    #our_utc_offset = time.timezone

    #timestamp = time.mktime(utc_tuple) - our_utc_offset
